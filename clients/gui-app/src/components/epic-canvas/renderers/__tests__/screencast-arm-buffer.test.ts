...
function downAt(
  ...
  return { payload, castSequence, clientX, clientY };
}
...
describe("createScreencastArmBuffer", () => {
  it("stores a down and matching primary nearby up and delivers both when current", () => {
    ...
    const buffer = createScreencastArmBuffer<string>(clock);
  ...
  it("keeps the pending gesture when noteMove stays within slop", () => {
    ...
    expect(buffer.hasPending()).toBe(true);
    expect(buffer.takeIfCurrent(7)).toEqual({ down: "down", up: null });
  });

  it("ignores a non-primary up and leaves the down pending", () => {
    const { clock } = createFakeClock();
    const buffer = createScreencastArmBuffer<string>(clock);
  ...
  it("ignores a far up and leaves the down pending", () => {
    const { clock } = createFakeClock();
    const buffer = createScreencastArmBuffer<string>(clock);
  ...
  it("ignores a second storeDown so there is no queue", () => {
    ...
    expect(buffer.takeIfCurrent(7)).toEqual({ down: "first", up: null });
  });
  ...
  it("drop clears the timeout and the pending gesture", () => {
    ...
    advance(SCREENCAST_ARM_BUFFER_TIMEOUT_MS - 1);
    expect(buffer.takeIfCurrent(9)).toEqual({ down: "second", up: null });
  });
});
