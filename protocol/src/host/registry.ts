...
export const providersListV70 = defineRpcContract({
  ...
  // The LIVE schemas, because this is the latest line - the same rule every
  // other method's newest contract follows, and the reason growth on an
  // unreleased line needs no bridge.
  //
  // The hand-frozen `...V70` copies still exist beside them, held identical by
  // an equality guard in `provider-model-providers-compat.test.ts`. They are
  // the snapshot this line will be pinned to the moment it is released or a
  // newer major opens above it - written in advance because v5.0 and v6.0 were
  // each still pointing at live schemas on the day a release shipped them, and
  // both grew a released line before anyone noticed. Cutting the snapshot
  // early does not stop growth (growth is legal here); it means the pin is a
  // one-line change against a copy that is already correct, rather than an
